/**
 * Match kickoff times in source data are stadium-local wall clock (MM/DD/YYYY HH:mm).
 * Convert to UTC ms so clients can format in the viewer's device timezone.
 */
...
function getKickoffTimestamp(game) {
    if (!game) return 0;
...
function enrichGameKickoff(game) {
    if (!game) return game;
